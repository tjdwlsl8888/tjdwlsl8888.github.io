...
<div class="head">  
``` 
&emsp;-> 이러면 아하 머리부분 블록이구나 하고 내가 지정하거나 판독할 수 있다. 
  
```html
<div class="body">
```
&emsp;-> 이렇듯 직관적으로 구분하기 쉽게 명칭을 넣어주는게 좋다. 
<div class="blank-space"></div>
...
작성법: 특정 태그 안에 `id="명칭"`식으로 고유한 이름표를 붙이는 느낌으로 이해하면 편하다.
<br>
...
```
<div class="blank-space"></div>
위의 예시 코드는 뭔가 복잡해보이지만 여기서 중점적으로 봐야할건 `#wrap` 이 부분과 `띄어쓰기` `div.head` 이거다. 
<br><br>
...
<br><br>
`div.head` 이 부분은 위에서 배운 클래스 선택자를 떠올려보면 된다. 
<br>
조금 다른 부분은 앞에 어떤 태그인지까지 적고 그 다음 . 을 찍고 클래스명을 적었다.
<br><br>
```html
<div id="wrap"> 부모 안에
 ->  <div class="head">  하위 태그 
```
<br>
이렇게 파고 들어가는 구조를 입력할 때는 어떤 태그의 클래스 명인지 명확하게 적어줘야한다. 
<br><br>
그리고 선택자 사이에 **띄어쓰기**가 있는데 `선택자 -> 선택자` 이렇게 해당 하위 태그로 들어갈 수 있다.
<br>
(맛보기로 직계자손은 >, 그냥 후손들은 띄어쓰기로 안으로 파고 들어 지정할 수 있다.)
<br><br>
```css
#wrap div.head {}
```
<br>
이 개념들을 차례로 정리하면, id가 wrap인 태그 안에 있는 하위 태그 들 중 
<br>
div태그를 달고있는 클래스 명이 head인 이 녀석(?)을 선택해서 작업하고 싶을 때 이렇게 입력하면 된다. 
...
작업할 때 굉장히 유용한 팁이다. 완전히 똑같은 디자인을 여러 태그에 동시에 주고 싶을 때는 코드를 여러 번 쓰지 말고, **쉼표( , )**로 구분해서 한 번에 묶어버리면 된다.
